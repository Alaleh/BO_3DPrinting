...
def generate_feasible_points(n_ranges):
    data_points = []
    value_ranges = [[4, 155, 115], [4.1, 156.5, 116.5], [4.2, 158, 118], [4.3, 159.5, 119.5], [4.4, 161, 121],
                    ...
                    [6.1, 181.5, 141],[6.2, 183, 142], [6.3, 184.5, 143], [6.4, 186, 144], [6.5, 187.5, 145], [6.6, 189, 146],
                    [6.7, 190.5, 147], [6.8, 192, 148], [6.9, 193.5, 149], [7, 195, 150], [7.1, 196, 151],
                    [7.2, 197, 152],[7.3, 198, 153], [7.4, 199, 154], [7.5, 200, 155], [7.6, 201, 156], [7.7, 202, 157],
                    [7.8, 203, 158],[7.9, 204, 159], [8, 205, 160], [8.1, 207, 161.5], [8.2, 209, 163], [8.3, 211, 164.5],
                    [8.4, 213, 166],[8.5, 215, 167.5], [8.6, 217, 169], [8.7, 219, 170.5], [8.8, 221, 172], [8.9, 223, 173.5],
                    [9, 225, 175], [9.1, 226, 177], [9.2, 227, 179], [9.3, 228, 181], [9.4, 229, 183], [9.5, 230, 185],
                    [9.6, 231, 187], [9.7, 232, 189], [9.8, 233, 191], [9.9, 234, 193], [10, 235, 195], [10.1, 235.5, 195.5],
                    [10.2, 236, 196], [10.3, 236.5, 196.5], [10.4, 237, 197], [10.5, 237.5, 197.5], [10.6, 238, 198],
                    [10.7, 238.5, 198.5], [10.8, 239, 199], [10.9, 239.5, 199.5], [11, 240, 200], [11.1, 240, 200.5], [11.2, 240, 201],
                    [11.3, 240, 201.5], [11.4, 240, 202], [11.5, 240, 202.5], [11.6, 240, 203], [11.7, 240, 203.5],
                    [11.8, 240, 204], [11.9, 240, 204.5], [12, 240, 205], [12.1, 240, 205], [12.2, 240, 205],
                    [12.3, 240, 205], [12.4, 240, 205], [12.5, 240, 205], [12.6, 240, 205], [12.7, 240, 205],
                    [12.8, 240, 205], [12.9, 240, 205], [13, 240, 205], [13.1, 240.5, 205.5], [13.2, 241, 206],
                    [13.3, 241.5, 206.5], [13.4, 242, 207], [13.5, 242.5, 207.5], [13.6, 243, 208],
                    [13.7, 243.5, 208.5], [13.8, 244, 209], [13.9, 244.5, 209.5], [14, 245, 210], [14.1, 245.5, 211], [14.2, 246, 212],
                    [14.3, 246.5, 213], [14.4, 247, 214], [14.5, 247.5, 215], [14.6, 248, 216], [14.7, 248.5, 217],
                    [14.8, 249, 218], [14.9, 249.5, 219], [15, 250, 220]]
...
def main():
    ...
    train_x, train_y = read_input_output()
    ...
    acq_func = qExpectedHypervolumeImprovement(model=model, ref_point=ref_point.tolist(), partitioning=partitioning, )

    # acquisition function optimization - continuous
    # new_input, acq_vals = optimize_acqf(acq_function=acq_func,
    #                                     bounds=torch.tensor([[0, 0], [1, 1]], dtype=torch.float32), q=1,
    #                                     num_restarts=10, raw_samples=1024)

    # acquisition function optimization - discrete
    feasible_points = generate_feasible_points(input_ranges)
    new_input, acq_vals = optimize_acqf_discrete(acq_function=acq_func, q=1,
                                                 choices=feasible_points, unique=True, dtype=torch.float32)

    # print(new_input, acq_vals)

    train_x_to_evaluate = []

    for i in range(len(input_ranges)):
        train_x_to_evaluate.append(np.round(input_ranges[i][0] + (input_ranges[i][1] - input_ranges[i][0]) * new_input[0][i].item(), 1))
